...
const handleMarkerClick = (location) => {
  ...
  const { bookmarks } = state.settings;
  summaryContainer.innerHTML = `<div data-id="${location.id}" class="mdc-ripple-surface pointer location-summary" style="background-image: linear-gradient( rgb(0 0 0 / 0.6), rgb(0 0 0 / 0.6) ),url('${buildfire.imageLib.cropImage(location.listImage ? location.listImage : cdnImage(location.listImage),{ size: "full_width", aspect: "16:9"})}');background-color: ${location.listImage ? 'unset': 'currentcolor'}">
