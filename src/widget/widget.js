...
const initEventListeners = () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (e.target.id === 'searchLocationsBtn') {
      ...
      state.selectedLocation = listLocations.find((i) => i.id === e.target.dataset.id);
      showLocationDetail();
...
const initMainMap = () => {
  ...
  if (design.defaultMapStyle === 'dark') {
    options.styles = options.styles.concat(darkMapStyle);
  }
    ...
    fetchSettings(() => {
      ...
      if (d.listViewPosition !== o.listViewPosition || d.listViewStyle !== o.listViewStyle) {
        ...
        if (listLocations.length > 0) {
          [state.selectedLocation] = listLocations;
          showLocationDetail();
        }
      } else {
        hideFilterOverlay();
