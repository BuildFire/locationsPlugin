      ...
      Promise.all(promises)
        .then((result) => {
          ...
          if (selectedLocation.images?.length > 0) {
            if (pageMapPosition === 'top') {
              selectors.cover.style.backgroundImage = `linear-gradient( rgb(0 0 0 / 0.6), rgb(0 0 0 / 0.6) ),url('${buildfire.imageLib.cropImage(selectedLocation.images[0].imageUrl, { size: "xl", aspect: "16:9", mode: 'entropy' })}')`;
              selectors.cover.style.display = 'block';
            } else {
              selectors.main.style.backgroundImage = `linear-gradient( rgb(0 0 0 / 0.6), rgb(0 0 0 / 0.6) ),url('${buildfire.imageLib.cropImage(selectedLocation.images[0].imageUrl, { size: "xl", aspect: "16:9", mode: 'entropy' })}')`;
            }
          }
